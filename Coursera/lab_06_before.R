...
#Объединим full_name fraction name
...
VRU8_long <- gather(VRU8, key = full_name, value = X61994.2226908, -fraction_id, -fraction_name)
str(VRU8_long)
VRU8_long2 <- gather(VRU8.2, "voting", "vote", -full_name)

VRU8_long2 <- dplyr::mutate(VRU8_long2, vote_rez = ifelse (grepl("1.Yes", vote), 1, ifelse(grepl("2.No", vote), -1, ifelse(grepl("4.Did not vote", vote), 0, ifelse(grepl("0.Absent", vote), -0.5, ifelse(grepl("3.Abstain", vote), -0.25, 0))))) )

VRU8_long2 <- drop_na(VRU8_long2)

VRU8_long2_no_MP <- MP %>% column_to_rownames(var = 'Country')

k_means_VRU8 <- kmeans(VRU8_long2, centers = 4)

library(factoextra)

fviz_cluster(object = k_means_VRU8, data = VRU8_long2,
             ellipse.type = 'convex')
